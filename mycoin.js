...
histories.forEach(trans => {
    ...
    tr.innerHTML = `
        <td>${trans.proccess === 'get' ? `-${trans.amount}` : `+${trans.amount}`}</td>
        <td>${trans.dated}</td>
...
function makeTransaction(e) {
    ...
    if (validity) {
        let targ = new Saved(amountInput.value, e.toLowerCase(), date, remark.value);
        targ.total();
        targ.history(targ);
    }
}
...
modalBtn.addEventListener('click', e => {
    makeTransaction(e.target.textContent);
});
